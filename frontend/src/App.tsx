  ...
  return (
    <Router>
      <div className="min-h-screen bg-background text-foreground antialiased selection:bg-primary/10">
        <Navbar isAdminLoggedIn={isAdminLoggedIn} onLogout={() => setIsAdminLoggedIn(false)} />
        ...
        <main className="w-full">
          <Routes>
            ...
            <Route
              path="/admin"
              element={isAdminLoggedIn ? <Navigate to="/admin/dashboard" replace /> : <AdminLogin onLoginSuccess={() => setIsAdminLoggedIn(true)} />}
            />
